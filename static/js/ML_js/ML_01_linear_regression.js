...
desc: 
`
<span style="font-size:20px; font-weight:bold;">此段程式碼為 Python 的套件匯入指令
（import），逐行解釋如下：<br><br></span>

<span style="font-size:16px; font-weight:bold;">import numpy as np</span><br>
匯入 NumPy 套件，簡稱 np。<br>
NumPy 提供高效能的數值運算、矩陣運算、陣列操作等功能。<br>
例子：<code>arr = np.array([1,2,3])</code><br><br>

<span style="font-size:16px; font-weight:bold;">import pandas as pd</span><br>
匯入 Pandas 套件，簡稱 pd。<br>
Pandas 用來處理表格資料（DataFrame / Series），非常適合資料清理與分析。<br>
例子：<code>df = pd.read_csv("data.csv")</code><br><br>

<span style="font-size:16px; font-weight:bold;">import seaborn as sns</span><br>
...
<span style="font-size:16px; font-weight:bold;">import plotly.express as px</span><br>
匯入 Plotly Express，簡稱 px。<br>
Plotly 提供互動式圖表功能，比 Matplotlib 更適合網頁展示和互動。<br>
例子：<code>fig = px.scatter(df, x="age", y="salary")</code><br><br>

<span style="font-size:16px; font-weight:bold;">from matplotlib import pyplot as plt</span><br>
...
簡單說，這五行都是 Python 資料科學與視覺化常用套件，各自用途不同，但經常一起使用：<br>
<ul>
<li>numpy → 數值計算</li>
<li>pandas → 資料整理</li>
